...
# pacman_conf.py
#
# Based on pyalpm code Copyright © 2011 Rémy Oudompheng <remy@archlinux.org>
#
...
class PacmanConfig(collections.OrderedDict):
    ...
    def load_from_file(self, filename):
        for section, key, value in pacman_conf_enumerator(filename):
            if section == 'options':
                ...
                if key == 'Usage':
                    self.repos[section].setdefault('usage', value)
                elif key == 'Server':
    ...
    def apply(self, handle, updates=False):
        # File paths
        ...
        for repo, info in self.repos.items():
            ...
            if usage and ',' in usage:
                usage = [x for x in usage.split(',') if x in ['Update', 'All', 'Install']]
            if (updates and len(usage) > 0) or not updates:
